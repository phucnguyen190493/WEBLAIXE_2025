                ...
                <table class="table table-striped table-bordered table-hover text-center" id="example">
                    ...
                    <tbody>
                        ...
                        <tr class="odd gradeX">
                            ...
                            <td>{{ $bch["ten"] }}</td>
                            <td>
                                {{ $bch->BangLai->id }} | {{ $bch-> BangLai->ten }}
                            </td>
                            <td>
                                <input type="checkbox" @checked($bch["active"] == 1) readonly onclick="return false;"/>
                            </td>
                            <td style="display: inline-flex;">
                                <a href="admin/bocauhoi/update/{{$bch["id"]}}" class="btn btn-info btn-sm">
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Sửa
                                </a>
                                <button data-id="{{$bch["id"]}}" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal-delete">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">Xóa Bộ câu hỏi</h4>
        </div>
        <div class="modal-body">
            <form id="form-delete">
                {{ csrf_field() }}
                <input type="hidden" name="id" id="del-id">
                <p>Bạn có chắc muốn xóa Bộ câu hỏi với id <strong id="del-id"></strong> này?</p>
 ...
 <script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#modal-delete').on('show.bs.modal', function (event) {
          ...
          modal.find('.modal-body #delete').attr('href', 'admin/bocauhoi/delete/'+iddel);
        })
    });
 </script>
